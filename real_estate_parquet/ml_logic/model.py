...
from keras.layers import Input, Dense, Embedding, Flatten, Concatenate, BatchNormalization, Dropout, LeakyReLU
...
def initialize_keras_model(n_numeric_features: int) -> Model:
        ...
        unique_city_emb = Embedding(input_dim=33523, output_dim=32, name="unique_city_embedding")(unique_city_id_input)
        ...
        x = Dense(100, kernel_regularizer=reg)(merged)
        x = BatchNormalization()(x)
        x = LeakyReLU(alpha=0.01)(x)
        x = Dropout(rate=0.3)(x)

        x = Dense(75, kernel_regularizer=reg)(x)
        x = BatchNormalization()(x)
        x = LeakyReLU(alpha=0.01)(x)
        x = Dropout(rate=0.2)(x)

        x = Dense(50, kernel_regularizer=reg)(x)
        x = BatchNormalization()(x)
        x = LeakyReLU(alpha=0.01)(x)
